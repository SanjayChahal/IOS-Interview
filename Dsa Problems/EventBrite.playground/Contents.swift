...
wrapLines(words1, 13) "wrap words1 to line length 13" =>
  ...
  [ "The-day-began", // 13 chars -> cannot add anything else
    "as-still-as", // 11 chars -> we could add a one letter word
    "the-night",
    "abruptly",
    "lighted-with",
    "brilliant",
...
let words1 = ["The","day","began","as","still","as","the","night","abruptly","lighted","with","brilliant","flame"]
let words2 = ["Hello"]
let words3 = ["Hello", "Hello"]
let words4 = ["Well", "Hello", "world"]
let words5 = ["Hello", "HelloWorld", "Hello", "Hello"]
let words6 = ["a", "b", "c", "d"]
...
class Solution {
  ...
  func wrapWords(_ arrWords: [String], _ maxCount: Int) -> [String] {
    ...
    while index < arrWords.count {
      print("iterating \(index)")
      if let wordString = arrWords[index] as? String {
        if (singleString.count + wordString.count + 1 <= maxCount)  {
          print("here for counter \(index)")
          singleString = singleString.isEmpty ? wordString :  singleString + "-" + wordString
        } else {
                 print("appending in while--- \(singleString)")
          if index != 0 {
              returnArray.append(singleString)
          }
          singleString = ""
          singleString = wordString
        }
...
print(Solution().wrapWords(words3, 5))
